...
export function PageTOCTitle(props: ComponentProps<"h2">) {
    return (
        ...
        >
            ...
            {"Sur cette page"} {/* <I18nLabel label="On this page" /> */}
        </h3>
    );
}
...
export function PageArticle(props: ComponentProps<"article">) {
    return (
        <article
            {...props}
            className={cn(
                "flex min-w-0 w-full flex-col gap-4 pt-12 px-6 md:px-12 md:mx-auto",
                props.className
            )}
        >
            {props.children}
...
export function PageRoot({ toc = false, children, ...props }: RootProps) {
    const content = (
        <div
            ...
            {...props}
            className={cn(
                "grid md:grid-cols-[1fr_280px] w-full h-full mx-auto max-w-(--fd-page-width)",
                props.className
            )}
        >
            {children}
