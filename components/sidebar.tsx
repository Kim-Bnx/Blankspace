    ...
    "relative flex flex-row items-center gap-2 rounded-lg p-2 ps-(--sidebar-item-offset) text-start text-muted-foreground [overflow-wrap:anywhere] [&_svg]:size-4 [&_svg]:shrink-0",
    {
        variants: {
            active: {
                ...
                false: "transition-colors hover:bg-border hover:text-foreground hover:transition-none",
            },
            insideFolder: {
...
export function SidebarContent(props: ComponentProps<"aside">) {
    ...
    return (
        <aside
            ...
            className={cn(
                "left-0 rtl:left-auto border-r rtl:right-(--removed-body-scroll-bar-size,0) flex text-sm border-e max-md:hidden *:md:w-full h-full pl-12",
                /* collapsed && [
            ...
            onPointerLeave={(e) => {
                ...
                timerRef.current = window.setTimeout(
                    ...
                    Math.min(e.clientX, document.body.clientWidth - e.clientX) >
                        100
                        ? 0
                        : 500
                );
        ...
        >
            <div className="sticky max-h-[calc(100vh-6.5rem)] overflow-auto top-26">
                {props.children}
            </div>
        </aside>
    );
...
export function SidebarItem({
    ...
    return (
        <Link
            {...props}
            data-active={active}
            className={cn(itemVariants({ active }), props.className, "mr-4")}
            prefetch={prefetch}
        >
...
export function SidebarFolderLink(props: LinkProps) {
    ...
    return (
        <Link
            {...props}
            data-active={active}
            className={cn(itemVariants({ active }), "mr-4", props.className)}
            onClick={(e) => {
                if (
