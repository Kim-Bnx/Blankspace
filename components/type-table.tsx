...
export function TypeTable({ type }: { type: Record<string, TypeNode> }) {
    return (
        <div className="@container flex flex-col p-2 text-fd-card-foreground rounded-2xl border my-6 text-sm overflow-hidden">
            <div className="flex items-center not-prose px-3 py-1">
                <span className="w-[25%]">Prop</span>
    ...
    return (
        <Collapsible
            ...
            className={cn(
                ...
                open
                    ? "shadow-sm bg-dark-450 not-last:mb-2"
                    : "border-transparent"
            )}
        >
            <CollapsibleTrigger className="relative flex flex-row items-center w-full group text-start px-3 py-2 not-prose hover:bg-dark-450">
                <code
                    className={cn(
                        keyVariants({
                            deprecated,
